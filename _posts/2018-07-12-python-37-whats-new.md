...
파이썬에서 정적 타입 검사를 하려는 노력은 오래전부터 있었습니다.

처음 파이썬 3에서 `Type Annotation`이라는 문법이 등장해서 코드 상에 타입을 명시할 수 있게 되었지만, IDE가 해당 문법을 파싱해서 에러를 나타내어 주는 것 정도의 역할이었고 실질적인 기능은 없었습니다. PyCharm과 같은 IDE를 사용하지 않는 유저들은 혜택을 볼 수 없었구요.
...
```
 ...
 이후 파이썬 3.5 버전에서 [typing](https://docs.python.org/3/library/typing.html) 모듈이 등장하면서 복잡한 타입( Dict, List )을 다룰 수 있게 되었고, 파이썬의 창시자인 _Guido van Rossum_ 이 [mypy](https://github.com/python/mypy)를 내놓으면서 IDE 없이도 실질적인 정적 타입 검사가 가능해졌습니다. 파이썬 3.6에서는 Dict[int, Tuple] 와 같이 아주 복잡한 타입까지도 지원할 수 있게 되는 발전이 이루어졌습니다.

 그리고 대망의 이번 파이썬 3.7에서는 typing 모듈이 안고있던 두 가지 큰 과제를 해결하였습니다.
...
typing 모듈은 각 타입을 클래스를 사용하여 표현하고 있었기에 import가 아주 오래 걸리는 모듈이었습니다. 특히 generic class를 만드는 것에 아주 오랜 시간이 걸렸습니다. ([Github issue ](https://github.com/python/typing/issues/432), [Generic class](https://docs.python.org/3/library/typing.html#user-defined-generic-types))

그러나 이번 파이썬 3.7에서는 속도 부분에서 typing 모듈이 괄목할만한 발전을 이루었습니다.

- import 속도 7배
- user-defined generic class creation 속도 4배
- generic class instantiation 속도 5배

generic class와 관련되어 많은 발전을 이루어진 것이 큰 영향을 주었다고 하는데, 기술적인 부분에 대해 더 자세히 알고 싶으신 분은 [PEP560](https://www.python.org/dev/peps/pep-0560/)을 읽어보시면 도움이 될 것 같습니다.


2. __forward reference 방식의 부재__
...
기존 typing에서는 forward reference의 부재로 인하여 코드 순서상 정의되지 않은 클래스 타입을 참조할 수가 없었습니다.

이는 클래스가 자기 참조나 순환 참조라도 하게 되면 근본적으로 해결할 수 없는 문제였고,
...
이러한 forward referencing 문제를 해결하기 위하여 파이썬 4.0부터는 함수나 변수의 type annotation을 판정(evaluation)을 정의 시점이 아니라 뒤로 미룰 것이라고 하는데요.

이번 파이썬 3.7에서는 실험적으로 forward referencing이 지원되기 시작하였습니다.
...
\_\_future__ 모듈에서 `annotations`를 import하여 파이썬 3.7에서 forward referencing이 이루어지도록 할 수 있습니다.

## dataclasses

> PEP 557: Data classes

파이썬의 특징 중 하나는 [duck typing](https://nesoy.github.io/articles/2018-02/Duck-Typing)을 적극적으로 활용하여 클래스를 원하는 대로 set, dictionary, iterable처럼 쓸 수 있다는 것입니다.

이번 파이썬 3.7에서 추가된 [dataclasses](https://docs.python.org/3/library/dataclasses.html) 모듈은 클래스를 데이터로서 다루기에 편리한 속성들을 쉽게 추가할 수 있도록 하는 모듈입니다.

클래스를 데이터로 다루기 위해서 필요한 요소에는 주로 아래와 같은 것이 있습니다.
...
간단히 훑어보시면 알겠지만 굉장히 흔히 작성하게 되는 클래스입니다. 단순히 setter 역할을 하는 `__init__()`과, 거의 똑같은 `__eq__()` `__gt__()`와 같은 메소드들.

dataclasses는 이러한 반복 작업을 아주 간단하게 만들어 줍니다.

아래 클래스는 위의 클래스와 완전히 동일합니다.
...
클래스에 type annotations 문법을 이용하여 필드들을 정의해놓고, `@dataclass` 데코레이터를 붙여주면 자동으로 필요한 속성들이 생성됩니다.
...
필드에 `field()` 를 붙여주면 repr이나 order에서 사용하지 않게끔 제외하는 것도 가능합니다.

## asyncio / contextvars

아주 주관적인 의견으로 현재 파이썬에서 가장 핫한(?) 모듈이 `asyncio`가 아닐까 싶습니다. 파이썬에서 효율적인 비동기 연산을 수행하기 위해서 `threading`, `multiprocessing`부터 `concurrent.futures` 까지 여러 시도와 발전이 있었습니다. 그러나 GIL을 사용하는 파이썬의 근본적인 한계 때문에 스레딩이 제 힘을 발휘하기가 힘들었습니다.

그러한 상황에서 파이썬 3.4에서 등장한 coroutine 기반의 asyncio가 각광을 받은 것은 어찌보면 당연한 결과였습니다.

파이썬 3.7에서 asyncio와 관련된 주목할만한 변화는 크게 두 가지 입니다.
...
이 함수가 중요한 이유는 asyncio의 접근성을 높여주었기 때문이라고 저는 생각하는데요. event loop의 개념 자체가 낯선 저와 같은 주니어 개발자들에게 있어서 event loop을 생성하고 그것에 task를 등록하는 asyncio의 문법은 바로 직관적으로 다가오지 않았습니다.
...
asyncio.run()를 통하여 event loop에 대한 부분을 명시적으로 드러내지 않고도 코드를 짤 수 있게 되었습니다.
...
이는 스레드마다 로컬 변수의 값이 달라지는 [Thread-Local Storage](https://en.wikipedia.org/wiki/Thread-local_storage) 개념과 유사한데요. asyncio에서는 한 스레드에서 여러 비동기 I/O 연산이 수행되는 방식이기 때문에 이에 대한 해결책으로서 등장한 것이 contextvars라고 볼 수 있습니다.
...
파이썬 3.6 버전을 주로 쓰는 개발자라면 dict가 입력한 대로 순서를 보장한다고 알고 있을 수도 있는데, 이는 CPython 3.6 버전의 dict의 구현이 그렇게 되어 있을 뿐, 파이썬 스펙 상에는 순서에 대한 부분은 명시되어 있지 않았습니다. 즉 PyPy나 Jython에서도 동일하게 동작하는 것이 보장되어 있지 않았습니다.
...
파이썬 코드 디버깅을 위해서 `pdb` 모듈을 아래와 같이 이용하게 되는데요.
